...
internal sealed class UsageTracker : IDisposable
{
    ...
    public UsageTracker(AppSettings s,UsageData d){settings=s;data=d;timer.Tick+=Tick;timer.Start();}
    void Tick(object? sender,EventArgs e){var now=DateTime.Now;var sec=Math.Min(15,(now-last).TotalSeconds);last=now;if(Running&&Native.IdleSeconds()<settings.IdleMinutes*60){data.Add(Native.ActiveProcess(),sec);Updated?.Invoke();}}
